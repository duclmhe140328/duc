                        ...
                        <table style="background-color: pink" border="1">
                            ...
                            <c:forEach var="o1" items="${billss}">
                                <tr>
                                    ...
                                    <td><a href="billDetail?bid=${o1.bid}">${o1.bid}</a></td>
                                         <td>${o1.dateCreate}</td>
                                         <td>${o1.total}</td>
                                         <td>${o1.recName}</td>
                                         <td>${o1.address}</td>
                                         <td>${o1.recPhone}</td>
                                         <c:if test="${o1.status==0}">
                                             <td  style="font-weight: bold;color: blue">WAIT</td>
                                         </c:if>
                                             <c:if test="${o1.status==1}">
                                             <td  style="font-weight: bold;color: red">DONE</td>
                                         </c:if>

                                             <c:if test="${o1.status==2}">
                                             <td  style="font-weight: bold;color: green">PROCESS</td>
                                         </c:if>
                                          <c:if test="${o1.status==0}">
                                            <td><a href="updatestatus?status1=${o1.bid}">DONE</a>/<a href="process?bid=${o1.bid}">PROCESS</a></td>
                                             <td><a href="deletest?bid1=${o1.bid}">Cancel order</a></td>
                                         </c:if>
                                             <c:if test="${o1.status==1}">
                                            <td><a href="changeback?bid=${o1.bid}">WAIT</a>/<a href="process?bid=${o1.bid}">PROCESS</a></td>
                                            
                                         </c:if>
                                             <c:if test="${o1.status==2}">
                                            <td><a href="updatestatus?status1=${o1.bid}">DONE</a>/<a href="changeback?bid=${o1.bid}">WAIT</a></a></td>
